...
enum ProposalTargetOutputs {kRoiOutput, kLabel, kBboxTarget, kBboxWeight};
}
...
struct ProposalTargetParam : public dmlc::Parameter<ProposalTargetParam> {
  ...
  DMLC_DECLARE_PARAMETER(ProposalTargetParam) {
    ...
    DMLC_DECLARE_FIELD(batch_rois).describe("Number of ROIs for one image");
 ...
 public:
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    Tensor<xpu, 3, DType> xpu_rois      = in_data[proposal_target_enum::kRois].
                                          get_with_shape<xpu, 3, DType>(Shape3(num_image, num_roi, 5), s);
    ...
    TensorContainer<cpu, 2, DType> cpu_output_rois(Shape2(param_.batch_rois, 5), 0.f);
    TensorContainer<cpu, 1, DType> cpu_labels(Shape1(param_.batch_rois), 0.f);
    TensorContainer<cpu, 2, DType> cpu_bbox_targets(Shape2(param_.batch_rois, param_.num_classes * 4), 0.f);
    TensorContainer<cpu, 2, DType> cpu_bbox_weights(Shape2(param_.batch_rois, param_.num_classes * 4), 0.f);
    ...
    if (param_.ohem) {
        ...
        for (index_t i = 0; i < num_image; ++i) {
          ...
          SampleROI(kept_rois_i, 
                    ...
                    cpu_bbox_weights.Slice(i * num_roi_per_image, (i + 1) * num_roi_per_image));
        }
    }

    Tensor<xpu, 2, DType> xpu_output_rois  = out_data[proposal_target_enum::kRoiOutput].
                                             get_with_shape<xpu, 2, DType>(Shape2(param_.batch_rois, 5), s);
    Tensor<xpu, 1, DType> xpu_labels       = out_data[proposal_target_enum::kLabel].
                                             get_with_shape<xpu, 1, DType>(Shape1(param_.batch_rois), s);
    Tensor<xpu, 2, DType> xpu_bbox_targets = out_data[proposal_target_enum::kBboxTarget].
                                             get_with_shape<xpu, 2, DType>(Shape2(param_.batch_rois, param_.num_classes * 4), s);
    Tensor<xpu, 2, DType> xpu_bbox_weights = out_data[proposal_target_enum::kBboxWeight].
                                             get_with_shape<xpu, 2, DType>(Shape2(param_.batch_rois, param_.num_classes * 4), s);
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    Tensor<xpu, 3, DType> gt_bboxes = in_grad[proposal_target_enum::kGtBboxes].get<xpu, 3, DType>(s);
...
class ProposalTargetProp : public OperatorProperty {
 public:
  ...
  std::vector<std::string> ListOutputs() const override {
    return {"output", "label", "bbox_target", "bbox_weight"};
  }
  ...
  bool InferShape(std::vector<TShape> *in_shape,
    ...
    auto output_rois_shape = Shape2(param_.batch_rois, 5);
    auto label_shape = Shape1(param_.batch_rois);
    auto bbox_target_shape = Shape2(param_.batch_rois, param_.num_classes * 4);
    auto bbox_weight_shape = Shape2(param_.batch_rois, param_.num_classes * 4);
