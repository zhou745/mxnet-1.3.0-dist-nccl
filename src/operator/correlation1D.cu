...
#define correlation1D_CUDA_CHECK(condition) \
...
template <typename Dtype>
__global__ void Correlate1DDataBackward0(const int nthreads, int num, int item,
...
template <typename Dtype>
__global__ void Correlate1DDataBackward1(const int nthreads,
...
void Forward_gpu(
      ...
      int single_side, int max_displacement_, int kernel_size_,
...
void Backward_gpu(
      ...
      int top_width_, int pad_size_, int single_side,
    ...
    //  == Run kernel Backward 0
    for (int n = 0; n < num; n++) {
    Correlate1DDataBackward0<Dtype><<<gridSize, kMaxThreadsPerBlock, 0, stream0>>>(
        botThreadCount,
    ...
    for (int n = 0; n < num; n++) {
    Correlate1DDataBackward1<Dtype><<<gridSize, kMaxThreadsPerBlock, 0, stream1>>>(
        botThreadCount,
        num, n, top_width_, top_height_, top_channels_,
        max_displacement_, x_shift, neighborhood_grid_width_, kernel_radius_,
        stride1_, stride2_,
...
template<typename Dtype>
inline void Correlation1DForward(const Tensor<gpu, 4, Dtype> &out,
                               const Tensor<gpu, 4, Dtype> &data1,
                               const Tensor<gpu, 4, Dtype> &data2,
                               const Tensor<gpu, 4, Dtype> &tmp1,
                               const Tensor<gpu, 4, Dtype> &tmp2,
  ...
  cuda::Forward_gpu(out, data1, data2, tmp1, tmp2, top_channels_, top_height_,
                    top_width_, pad_size_, single_side, max_displacement_, kernel_size_,
                    neighborhood_grid_radius_, neighborhood_grid_width_, kernel_radius_,
                    stride1_, stride2_, stream, stream_tmp1, stream_tmp2);
}

template<typename Dtype>
inline void Correlation1DBackward(const Tensor<gpu, 4, Dtype> &out_grad,
                            const Tensor<gpu, 4, Dtype> &in_grad1,
                            const Tensor<gpu, 4, Dtype> &in_grad2,
                            const Tensor<gpu, 4, Dtype> &tmp1,
                            const Tensor<gpu, 4, Dtype> &tmp2,
  ...
  cuda::Backward_gpu(out_grad, in_grad1, in_grad2, tmp1, tmp2, top_channels_,
                      top_height_, top_width_, pad_size_, single_side,
...
namespace op {
template<>
Operator* CreateOp<gpu>(Correlation1DParam param) {
  return new Correlation1DOp<gpu>(param);
}
}  // namespace op
}  // namespace mxnet
